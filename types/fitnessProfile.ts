...
export const fitnessProfileSchema = z.object({
  ...
  fitnessLevel: z
    ...
    .describe('Get user information fitness level'),
  fitnessGoal: z
    .enum(['Building Muscle', 'Losing Fat', 'Improve endurance', 'General Fitness'])
    .describe('Get user information fitness goal'),
  healthAndPhysicalCapacity: z.string().describe('Get user physical limitations or past injuries'),
  workoutAccess: z
    .enum(['gym', 'home', 'gym & home'])
    .describe('Get user have access to a gym, home or both'),
  introduction: z
    .string()
    .describe(
  ...
  message: z
    .string()
    .describe(
      'Additional information to user if any, ask if theres any information or question to add, ' +
